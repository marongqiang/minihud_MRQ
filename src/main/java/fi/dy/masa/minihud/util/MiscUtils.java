        ...
        if (stackTag != null && stackTag.contains("BlockEntityTag", Constants.NBT.TAG_COMPOUND))
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (entityDataTag.contains("CustomName", Constants.NBT.TAG_STRING))
                {
                    ...
                    Text nameText = Text.Serializer.fromJson(beeName);
                    String nameStr = nameText != null ? nameText.getString() : beeName;
                    lines.add(Math.min(1, lines.size()), Text.translatable("minihud.label.bee_tooltip.name", nameStr));
                }
